...
@push('style_section')
    <style>
        ...
        .listing-detail-hero__actions a + a{
...
font-size: 18px !important;
        ...
        @media (max-width: 575.98px){
            ...
            .listing-detail-hero__mobile-image img{
                width: 100%;
                height: auto;
                border-radius: 16px;
                border: 6px solid #2f2f2f;
            ...
            <div class="listing-detail-hero__grid">
                <div class="listing-detail-hero__media">
                    ...
                    <div class="listing-detail-hero__left-info">
                        <h2 class="listing-detail-hero__left-title">
                            ...
                            @endphp
                            @if($cityOnly)
                                <span class="listing-detail-hero__location">{{ strtoupper($cityOnly) }}</span>
                            @endif
                        </h2>
                        <div class="listing-detail-hero__left-subtitle">{{ __('translate.Go to motorcheck for history check') }}</div>

                        <div class="listing-detail-hero__actions">
                            @auth('web')
                                <a href="mailto:{{ html_decode($dealer?->email) }}">{{ __('translate.Email') }}</a>
                                <a href="tel:{{ html_decode($dealer?->phone) }}">{{ __('translate.Call') }}</a>
                                @if(!empty($dealer?->id))
                                    <a href="{{ route('user.messages.start', $dealer->id) }}">Chat</a>
                                @endif
                            @else
                                <a href="{{ route('login') }}">{{ __('translate.Email') }}</a>
                                <a href="{{ route('login') }}">{{ __('translate.Call') }}</a>
                ...
                <div class="listing-detail-hero__right">
                    <div class="listing-detail-hero__meta">
                        <span>{{ html_decode($dealer?->name) }}</span>
                        @php
                        ...
                        @endphp
                        @if($__isDealerAd)
                            <span class="listing-detail-hero__badge">{{ __('translate.Dealer') }}</span>
                        @endif
                    </div>

                    <div class="listing-detail-hero__keyinfo">
                        @if($car->year)
                            <div><span class="kicon"><i class="fa-regular fa-calendar"></i></span>{{ html_decode($car->year) }}</div>
                        @endif
                        @if($car->engine_size)
                            <div><span class="kicon"><i class="fa-solid fa-gears"></i></span>{{ html_decode($car->engine_size) }}</div>
                        @endif
                        @if($car->transmission)
                            <div><span class="kicon"><i class="fa-solid fa-sliders"></i></span>{{ html_decode($car->transmission) }}</div>
                        @endif
                        @if($car->mileage)
                            <div><span class="kicon"><i class="fa-solid fa-gauge"></i></span>{{ html_decode($car->mileage) }}</div>
                        @endif
                        @if($car->body_type)
                            <div><span class="kicon"><i class="fa-solid fa-car-side"></i></span>{{ html_decode($car->body_type) }}</div>
                        @endif
                        @if($car->drive)
                            <div><span class="kicon"><i class="fa-solid fa-road"></i></span>{{ html_decode($car->drive) }}</div>
                        @endif
                        @if($car->interior_color)
                            <div><span class="kicon"><i class="fa-solid fa-couch"></i></span>{{ html_decode($car->interior_color) }}</div>
                        @endif
                        @if($car->exterior_color)
                            <div><span class="kicon"><i class="fa-solid fa-palette"></i></span>{{ html_decode($car->exterior_color) }}</div>
                        @endif
                        @if($car->fuel_type)
                            <div><span class="kicon"><i class="fa-solid fa-gas-pump"></i></span>{{ html_decode($car->fuel_type) }}</div>
                        @endif
                        @if($car->condition)
                            <div>
                                <span class="kicon"><i class="fa-regular fa-circle-check"></i></span>
                        ...
                        @endif
                        @if($car->seller_type)
                            <div><span class="kicon"><i class="fa-solid fa-user-tag"></i></span>{{ html_decode($car->seller_type) }}</div>
                        @endif
                    </div>
